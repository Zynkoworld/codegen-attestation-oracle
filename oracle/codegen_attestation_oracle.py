#!/usr/bin/env python3
"""codegen-attestation-oracle -- a deterministic attestation-integrity decider for the `codegen` knowledge domain.

An **oracle** *deterministically decides* the truth of a case. This one decides, for a `codegen` knowledge
capsule, whether it is a well-formed, provenance-carrying attestation. VALID iff: domain=='codegen', a non-empty
...
def decide(c):
    if c.get("domain") != "codegen": return "INVALID"
...
def _run_probes(path):
    ...
    print("codegen attestation oracle: probes=%d | recall=%.3f | false_positives=%d | verdict=%s"%(len(probes),recall,fp,"PASS" if ok else "FAIL"))
